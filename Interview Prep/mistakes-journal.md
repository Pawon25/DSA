...
- **2026-08-05 review:** Cold attempt dropped `private` from the access-modifier list (said default/protected/public only) — the most relevant one for "hiding data access." Corrected with design-vs-mechanism framing ("abstraction is what you hide, encapsulation is how you enforce it") and the List/ArrayList example. User said "got it" but did not verbally recite the corrected version back — recall is unconfirmed, not verified clean. Staying in box 2, needs a genuine cold-recall pass (ideally recited out loud, not just acknowledged) before promoting.
